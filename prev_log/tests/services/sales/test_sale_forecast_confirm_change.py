...
from prev_log.services.sales.change import confirm_sale_change_request
from prev_log.services.utils.log import ManualChangeStatus
from prev_log.tests.factory import ProductFactory, SaleForecastFactory
from prev_log.tests.factory.sales import SaleForecastOpenRequestFactory


class SaleForecastConfirmChangeTestCase(TestCase):
    ...
    def stub_forecast_request(
        ...
        # New request
        request = SaleForecastOpenRequestFactory(
            **self.filters,
            ano=target_date.year,
            mes=target_date.month,
    ...
    def test_confirming_change_to_invalid_request_will_raise(self):
        ...
        with self.assertRaises(PrevLogBadRequestException) as context:
            confirm_sale_change_request(params=params)

        exc = context.exception
        self.assertEqual(exc.status, 400)
        self.assertEqual(exc.error, "sale confirm request")
    ...
    def test_confirming_change_will_change_the_forecast(self):
        ...
        forecasted = SaleForecast.objects.filter(
            ano=target_date.year,
            mes=target_date.month,
        ).first()
        ...
        requested_all = SaleForecastOpenRequest.objects.filter().all()
