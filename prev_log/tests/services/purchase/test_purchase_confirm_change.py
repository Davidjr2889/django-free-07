...
from prev_log.services.purchase.change import confirm_purchase_change_request
from prev_log.services.utils.log import LogForecastOrigin, ManualChangeStatus
from prev_log.tests.factory import ProductFactory, PurchaseForecastFactory
from prev_log.tests.factory.purchase import PurchaseForecastOpenRequestFactory


class PurchaseForecastConfirmChangeTestCase(TestCase):
    ...
    def stub_forecast_request(
        ...
        # New request
        request = PurchaseForecastOpenRequestFactory(
            **self.filters,
            ano=target_date.year,
            mes=target_date.month,
    ...
    def test_confirming_change_to_invalid_request_will_raise(self):
        ...
        with self.assertRaises(PrevLogBadRequestException) as context:
            confirm_purchase_change_request(params=params)

        exc = context.exception
        self.assertEqual(exc.status, 400)
        self.assertEqual(exc.error, "purchase confirm request")
    ...
    def test_confirming_change_will_change_the_forecast(self):
        ...
        forecasted = PurchaseForecast.objects.filter(
            ano=target_date.year,
            mes=target_date.month,
        ).first()
        ...
        self.assertEqual(all_logs[0].origem_forecast, LogForecastOrigin.PURCHASE.value)

        # We shouldn't have that open request anymore
        requested_all = PurchaseForecastOpenRequest.objects.filter().all()
